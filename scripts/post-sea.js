...
// Copy dist folder so worker scripts are available
console.log('[post-sea] Copying dist runtime files...');
copyDir(distDir, join(stageDir, 'dist'));
...
const legacyDocsDir = join(rootDir, 'readmes');
...
// Clean up stage directory to match legacy workflow
safeRemove(stageDir);

console.log('\n[post-sea] Release artifacts ready:');
console.log(`  - ${zipPath}`);
if (isWindows) {
  console.log(`  - ${join(releaseDir, 'EZ-Game-Audio-Conversion.7z')}`);
}
console.log(
