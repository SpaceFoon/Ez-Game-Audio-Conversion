...
const finalize = async (failedFiles = [], successfulFiles = [], jobStartTime = Date.now()) => {
    ...
    if (successfulFiles && successfulFiles.length > 0) {
        console.log('    ', successfulFiles.length, '🚀 Successful Files 🚀', successfulFiles.map((file) => `✅ ${file.outputFile} `));
    }
    else {
        console.log('\n💀😭😢NO SUCCESSFUL CONVERSIONS.😢😭💀\n');
    }
    if (failedFiles && failedFiles.length > 0) {
        console.log(failedFiles.length, '🛑 Failed Files 🛑', failedFiles.map((file) => `❌ ${file.outputFile}`));
    }
    else {
        console.log(' 🚀🎉✨No conversions failed✨🎉🚀');
    }
    console.log(` 🌞🌈🌼 Log files are in: ${settings.outputFilePath} Have a nice day! 🌼🌈🌞\n`);
    function restartApp() {
    ...
    restartApp();
    const quit = () => {
        rl.question(chalk.blue('(☞ﾟヮﾟ)☞  Press Enter to restart program 🔀'), () => {
            rl.close();
            process.exit();
        });
    };
